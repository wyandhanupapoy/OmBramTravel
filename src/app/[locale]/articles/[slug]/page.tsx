...
export default async function ArticleDetailPage({ params }: { params: Promise<{ locale: string; slug: string }> }) {
  ...
  return (
    <main className="bg-paper pb-24">
      ...
      <article className="mx-auto max-w-[780px] px-7 pt-12"><p className="border-l-4 border-beacon pl-5 text-xl leading-relaxed text-pine-dark">{article.excerpt}</p>{article.paragraphs.map((paragraph) => <p key={paragraph} className="mt-7 text-lg leading-[1.8] text-ink">{paragraph}</p>)}<div className="my-10 rounded-xl border border-line bg-card p-6"><h2 className="font-display text-2xl uppercase text-pine-dark">Catatan perjalanan</h2><ul className="mt-4 list-disc space-y-2 pl-5 text-ink-soft">{article.tips.map((tip) => <li key={tip}>{tip}</li>)}</ul></div><div className="rounded-xl bg-pine-dark p-7 text-paper"><h2 className="font-display text-2xl uppercase text-beacon">Siap menyusun rute Bandung?</h2><p className="mt-2 text-white/75">Pilih paket berdasarkan destinasi, harga, dan kendaraan yang paling sesuai.</p><Link href={`/${locale}/tours`} className="mt-5 inline-flex rounded-lg bg-beacon px-5 py-3 font-display text-sm font-semibold uppercase tracking-wide text-pine-dark no-underline">Lihat paket tour</Link></div></article>
      <section className="mx-auto mt-16 max-w-[780px] px-7"><h2 className="font-display text-2xl uppercase text-pine-dark">Artikel terkait</h2><div className="mt-5 grid gap-4 sm:grid-cols-2">{relatedArticles.map((related) => <Link key={related.slug} href={`/${locale}/articles/${related.slug}`} className="rounded-xl border border-line bg-card p-5 no-underline hover:border-pine"><span className="font-mono text-[10px] uppercase text-rust">{related.category}</span><h3 className="mt-2 font-display text-xl text-pine-dark">{related.title}</h3></Link>)}</div></section>
    </main>
  );
}
